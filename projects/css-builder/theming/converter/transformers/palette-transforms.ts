...
paletteTransformers.push((css) => {
    for (const palette of palettes) {
        ...
        for (const colorKey of paletteColorKeys) {
            css = css.replaceAll(
                ThemeTokens[palette][colorKey],
                `var(--theme-${mdcName}-${colorKey})`
            );
            const contrastKey = `${colorKey}-contrast`;
            css = css.replaceAll(
                ThemeTokens[palette][contrastKey],
                `var(--theme-${mdcName}-${contrastKey})`
            );
        }
    }
    return css;
...
paletteTransformers.push((css) => {
    return css.replaceAll(
        `${ThemeTokens.primary['default-contrast']}`,
...
paletteTransformers.push((css) => {
    return css.replaceAll(
        `var(--mdc-theme-text-primary-on-background, white)`,
        `var(--theme-primary-contrast)`
    );
});
...
paletteTransformers.push((css) => {
    return css.replaceAll(
        `var(--mdc-theme-secondary, ${ThemeTokens.accent.default})`,
        `var(--theme-secondary)`
    );
});
...
paletteTransformers.push((css) => {
    return css.replaceAll(
        `${ThemeTokens.accent['default-contrast']}`,
...
paletteTransformers.push((css) => {
    return css.replaceAll(
        `var(--mdc-theme-error, ${ThemeTokens.warn.default})`,
        `var(--theme-error)`
    );
});
...
paletteTransformers.push((css) => {
    return css.replaceAll(
        `${ThemeTokens.warn['default-contrast']}`,
...
paletteTransformers.push((css) => {
    return css.replaceAll(
        `var(--mdc-theme-text-error-on-background, rgba(255, 255, 255, 0.7))`,
        `var(--theme-error-contrast)`
    );
});
